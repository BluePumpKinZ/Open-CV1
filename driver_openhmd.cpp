...
static const char * const k_pch_Sample_PoseOffsetX_Float = "poseOffsetX";
...
{
    const float half_radians = degrees * static_cast<float>(M_PI) / 360.0f;
    return HmdQuaternion_t{cosf(half_radians), 0.0f, sinf(half_radians), 0.0f};
}

static HmdQuaternion_t MultiplyQuaternion(const HmdQuaternion_t& a, const HmdQuaternion_t& b)
{
    return HmdQuaternion_t{
...
{
    if (g_poseOffsets.yawDegrees != 0.0f) {
        const float radians = g_poseOffsets.yawDegrees * static_cast<float>(M_PI) / 180.0f;
        const float c = cosf(radians);
        const float s = sinf(radians);
        const float x = pose.vecPosition[0];
        const float z = pose.vecPosition[2];

        pose.vecPosition[0] = (c * x) + (s * z);
        pose.vecPosition[2] = (-s * x) + (c * z);
        pose.qRotation = MultiplyQuaternion(MakeYawQuaternion(g_poseOffsets.yawDegrees), pose.qRotation);
    }

    pose.vecPosition[0] += g_poseOffsets.x;
    pose.vecPosition[1] += g_poseOffsets.y;
    pose.vecPosition[2] += g_poseOffsets.z;
}

static void LoadPoseOffsets()
{
    g_poseOffsets.x = vr::VRSettings()->GetFloat(k_pch_Sample_Section, k_pch_Sample_PoseOffsetX_Float);
